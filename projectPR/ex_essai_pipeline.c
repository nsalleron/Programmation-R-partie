...
struct ddosChecker{
	int second;
	int secondAvantAutorisation;
	long tailleOctetEnvoyé;
	struct in_addr ipAddr;
};
	...
	while(((n = read(connect,CMD_BRUT,TAILLE)) !=0) && bThreadActif == 1){
		...
		if(n<0){
			if(errno == EINTR) continue; //Intéruption d'appel, on continue
				...
				/*Attente de l'ensemble des threads avant de finir */
				for(int i = 0; i< tailleThreadRequest;i++)
					pthread_join(arrayRequestThread[tailleThreadRequest], (void*)0);
		...
		CMD_COMPLET = strtok(CMD_BRUT, "\n");
		
		/*Uniquement pour du test de pipeline, on ne souhaite pas la deuxième ligne des navigateurs */
		while ( CMD_COMPLET != NULL && CMD_COMPLET[0] == 'G') {
			
			printf("CMD_COMPLET : %s\n",CMD_COMPLET);
			
			CMD = strtok_r(CMD_COMPLET, " ",&saveptr1);
			if(CMD == NULL)
				continue;
			...
			/*	Création du thread */
			pthread_create(&pidThread,NULL,funcRep,(void*)rep);
			...
			CMD_COMPLET = strtok(NULL, "\n");
			printf("CMD_COMPLET FIN : %s\n",CMD_COMPLET);
		}
		memset(CMD_BRUT, 0, TAILLE);
...
void *funcRep(void *arg){
	...
	printf("%d de %lu | THREADREP DEBUT TRAITEMENT de %s\n",connect,(long)info.infoReq.pthreadid,info.PATH_FILE);
	...
	/* Valeur de retour pour les forks */
	int returnValue = 0;
	...
	if(!strcmp(info.CMD,"GET")){
		...
		if(!bErreur){
			...
			if(!bRepertory){
				...
				/* fils ici */
				if(pidFils == 0){
					printf("%d | exécution finale du fichier : ls -l sur %s\n",getpid(),info.PATH_FILE);
					dup2(localPipe[1], STDOUT_FILENO);
					dup2(localPipe[1], STDERR_FILENO);
					close(localPipe[0]);
					close(localPipe[1]);
					execlp("ls", "ls", "-l", info.PATH_FILE, NULL);
					perror("execlp");
					exit(EXIT_FAILURE);	
				}else{
					close(localPipe[1]);
					...
					if(returnValue == 0){
						...
						while(1){
							n = read(localPipe[0], DATA_FILE,TAILLE);
							//printf("%lu | Impression data %d sur socket %d\n",(long)getpid(),n,connect);
							if (n < 0)
								perror("read ");
							if(n == 0)
...
int main(int argc, char *argv[]) {
	...
	while(1){
		...
		if(nbTotalConnexion < MAXCLIENT){
			...
			pthread_create(&pidThread,NULL,funcTraitement,(void*)info);
			pthread_mutex_lock(&mutexConnexion);
